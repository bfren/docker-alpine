...
def apply_mod_type [
    ...
    if ($base | path exists) and ($base | path type) == "dir" { ^find $base -type $type -exec chmod $mode {} + }
}
...
def apply_own_type [
    ...
    if ($base | path exists) and ($base | path type) == "dir" { ^find $base -type $type -exec chown $owner {} + }
}
