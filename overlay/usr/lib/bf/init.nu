...
export def main [] {
    print debug "Initialising container." init
    if ($init_d | path exists) { ls $init_d | get name | sort | each { |x| execute $x } }
    print debug "Init complete." init
}
...
def execute [
    ...
    print $"($filename): Running..." init/execute
    nu $"($init_d)/($filename)"
    print debug $"($filename): Done." init/execute
}
...
export def image [] {
    ...
    # output image info
    $"bfren | ($image)" | figlet
    print $"bfren ($image) v($version) \(alpine/($alpine)\)" init/image
    print $"https://github.com/bfren/docker-($image)" init/image
}
